<section class="sidebar">
    ...
    <ul class="sidebar-menu" data-widget="tree">
        ...
        if($setting['value']=='ya'){
            // cari level user; stop early jika session kosong supaya query tidak error
            $id_user_level = (int) $this->session->userdata('id_user_level');
            if($id_user_level){
                $sql_menu = "SELECT * 
                FROM tbl_menu 
                WHERE id_menu in(select id_menu from tbl_hak_akses where id_user_level=?) and is_main_menu=0 and is_aktif='y'";
                $main_menu = $this->db->query($sql_menu, [$id_user_level])->result();
            } else {
                $main_menu = [];
            }
        }else{
